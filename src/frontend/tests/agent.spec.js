...
test.describe('Add Agent modal', () => {
    ...
    test('Form starts empty before a template is selected', async ({ appPage }) => {
        await appPage.locator('button.button', { hasText: 'Add Agent' }).click();
        const modal = getModal(appPage);
        const nameInput = getFormGroup(modal, 'Name').locator('input');
        const endpointUrlInput = getFormGroup(modal, 'Endpoint URL').locator('input');
        ...
        await expect(endpointUrlInput).toHaveValue('');
        await expect(responsePathInput).toHaveValue('');
    ...
    test('Re-opening the modal resets the form to empty values', async ({ appPage }) => {
        await appPage.locator('button.button', { hasText: 'Add Agent' }).click();
